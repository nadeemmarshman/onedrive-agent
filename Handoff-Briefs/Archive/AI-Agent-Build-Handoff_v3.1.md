...
| v3.1 | 2026-06-26 | Re-added the Document Integrity Control (hash log) section, which was introduced in v2.2 but not present in v2.3/v3.0 since those branches were built from the Phase 1/2 completion work without that v2.2 addition merged in; backfilled v2.2 and v3.0 as closed hash log entries; no other content changes | Reconciling a continuity gap, not introducing new scope — re-adding a previously-defined practice = MINOR |
...
## Standing rule: document integrity control (hash log)

**Control objective:** I need assurance that the version of this handoff document referenced in any given chat session is the authentic, unmodified version — so a hash-verification step is applied as part of the version control process. This mirrors file integrity monitoring (FIM) controls used in change management, audit trails, and regulatory evidence handling — applied here at an appropriately small, personal-project scale, not as a claim of a regulated control environment.

**Why SHA-256, not MD5:** MD5 is fast but has known collision vulnerabilities (two different files can theoretically produce the same hash), so it is no longer trusted for integrity-critical use. SHA-256 is the current standard for software checksums, blockchain, and regulatory file-integrity requirements, and is the hash cited below as the defensible reference value.

**Process:** Each time a new version of this document is finalized, compute its SHA-256 hash and add a row to the log below. Before relying on a copy of this document (e.g. confirming the Project knowledge file matches what was last generated), recompute the hash of that copy and compare it against the logged value — a match confirms the file is byte-identical and unmodified; any difference means something changed and should be investigated before proceeding.

**Continuity note:** This section was introduced in v2.2 but was not present in v2.3/v3.0, since those branches were built from the Phase 1/2 completion work without that v2.2 addition merged in. Re-added here in v3.1, with the gap backfilled below where the hash could still be computed.

**Hash Log:**

| Version | Filename | SHA-256 Hash | Date Hashed | Verified Against | Result |
|---|---|---|---|---|---|
| v2.1 | AI-Agent-Build-Handoff_v2.1.md | `96f081e7d77ef4dee399d65e6db3bed3946e206edc8e7dcbc391c596d579f931` | 2026-06-26 | User's local PowerShell `Get-FileHash` output | Match (True) |
| v2.2 | AI-Agent-Build-Handoff_v2.2.md | `3838cf96dc8db94d50f0f2bd9687b929699c19b5ed977bd536b4e7a50eb95162` | 2026-06-26 | Not separately verified locally | Logged, not verified |
| v3.0 | AI-Agent-Build-Handoff_v3_0.md | `1c76c05557b0e0736a0f01e931356d3ab73fe914d616ea733df35fc663602d65` | 2026-06-26 | Hash of file as uploaded to chat | Logged, not verified |

**How to verify a file's hash (Windows PowerShell):**
```
Get-FileHash -Algorithm SHA256 "AI-Agent-Build-Handoff_vX.X.md"
```
Compare the output against the logged hash for that version. Case differences (upper/lowercase hex) don't matter — only the actual hex digits need to match.
