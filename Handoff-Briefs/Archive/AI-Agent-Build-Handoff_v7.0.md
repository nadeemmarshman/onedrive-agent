...
**Status: Phases 1–7 complete; Phase 8 (User Acceptance Testing / Pilot, UAT/Pilot) in progress — Definition of Ready (DoR) gate met and verified (2026-07-05), pilot run pending. Repo public at `github.com/nadeemmarshman/onedrive-agent` @ `733cc7d` (+ I7-close commit). Independent Cowork peer review completed and all findings remediated (2026-07-02). Resume bullets drafted (private).**
...
Testing / Pilot, UAT/Pilot) is now in progress: the Definition of Ready
(DoR) gate is met and verified (2026-07-05); the pilot run itself is the
one outstanding step.** The repo is public at
`github.com/nadeemmarshman/onedrive-agent` @ `733cc7d` (+ I7-close commit).

Next concrete step: **run the Phase 8 pilot**, per `TEST_BED_AND_CASES.md`:
1. Build the test bed at `C:\OneDrive-Agent_TestBed\` as specified in
   `TEST_BED_AND_CASES.md` (a controlled, contained real-data subset — not
   the full OneDrive folder)
2. Dry-run first via `agent_loop.py` (decide → propose only; no destructive
   action — the approval gate is not invoked from this script)
3. Live run via `test_phase5_live.py` (this is the script that invokes the
   approval gate; `agent_loop.py` does not — don't conflate the two)
4. Review the pre-run snapshot (`pre_run_snapshot.json`) and the proposed
   actions carefully before approving anything
5. Sign off on the pilot before considering expansion to the full OneDrive
   folder. **Pilot sign-off is the trigger to notify the Job Search project**
   (see sync contract) — reaching DoR is not.
...
- Phase 8 (UAT/Pilot): DoR met, pilot run pending — pick up by running the pilot per `TEST_BED_AND_CASES.md` unless told otherwise
...
## Standing rule: define acronyms and abbreviations on first use

Adopted 2026-07-05. Do not assume familiarity with the acronyms or abbreviations of any framework, method, or tool used in this project. On first use in any given piece of work, write the term in full with the short form in brackets — e.g. "Definition of Ready (DoR)", "User Acceptance Testing (UAT)", "Governance, Risk and Compliance (GRC)", "Risks, Assumptions, Issues, Dependencies (RAID)", "Software Development Life Cycle (SDLC)". This keeps every artifact self-explanatory to whoever reads it next — recruiter, hiring manager, or {{owner}} himself in a later session — without assuming they carry the same shorthand. (Applies going forward and to new content; the existing body of this document is not retro-expanded wholesale unless requested, to avoid a large low-value diff — but new sections and future edits follow the rule.)
