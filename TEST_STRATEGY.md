...
| **Code-catchable, added post-pilot** | **I27** — real personal file paths committed to a then-public repo once the live-run phases switched the agent's input from synthetic `sample_data/` to real personal folders, a risk-class change the pre-pilot design (above) never anticipated. Recurred in smaller forms as I29 (guard's own placeholder-masking bug) and I30 (substring-matching false positive). | **ADD a pre-commit privacy guard**: `check_staged_for_personal_data.py` + `.githooks/pre-commit`, two independent detection layers (known-token + structural pattern), covered by `test_personal_data_guard.py`. Same systemic-fix principle as the fixture-integrity guard, but discovered live rather than at pre-pilot design time — see `LESSONS_LEARNED.md` §2 lessons 8–10 for the guard's own evolution. |
...
- [ ] **Named-term scrub-check:** when a request specifically names terms to keep out of a commit (a person, a matter, a reference number), grep the staged files for those exact terms as an independent manual check before committing — the automated privacy guard (REQ-23) only catches terms already in its token list, not a just-stated instruction about something new (`LESSONS_LEARNED.md` §2 lesson 10).
